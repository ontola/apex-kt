...
class ResultSetParser(val set: Query) {
    private val documents = HashMap<Int, Document>()
    private val linkedResources = Resources.alias("lr")

    fun parse(): HashMap<Int, Document> {
        for (resultRow in set) {
            println("row: $resultRow")
            val document = ensureDocument(resultRow)
    ...
    private fun ensureDocument(row: ResultRow): Document {
